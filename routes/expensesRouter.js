...
router.get('/add', checkAuth(true), (req, res, next) => {
  res.send(render({ component: components.add, user: req.user }));
});
...
router.post('/add', checkAuth(true), async (req, res, next) => {
  ...
  if (error) {
    res.send(render({ component: components.add, user: req.user, error, data }));
    return;
  }
...
router.get('/information/:id', checkAuth(true), async (req, res, next) => {
  const {
    user,
    params: { id: dataId },
  } = req;
  ...
  res.send(render({ component: components.information, user, path: '../', data: result }));
});
